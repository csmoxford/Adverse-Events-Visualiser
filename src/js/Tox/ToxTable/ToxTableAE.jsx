...
const ToxTableAE = (props) => {
  ...
  const aeHeader = data.treatment.map((t,i) => [
    ...
    const totals = data.treatment.map((t,j) => {
      ...
      return [
        <td key={`ae${j}_0`} style={style}>{count1} {bracketPercent(count1,numberOfPatients[j])}</td>,
        <td key={`ae${j}_1`} style={style}>{count3} {bracketPercent(count3,numberOfPatients[j])}</td>
      ]
    })
  ...
  const rows = rowData.sort(sortFun).map((a,i) => {
    const cells = data.treatment.map((t,j) => {
      const style = {backgroundColor: t.color + '40'}
      return [
        <td key={`ae${j}_0`} style={style}>{a.treatmentData[j].count1} {bracketPercent(a.treatmentData[j].count1,numberOfPatients[j])}</td>,
        <td key={`ae${j}_1`} style={style}>{a.treatmentData[j].count3} {bracketPercent(a.treatmentData[j].count3,numberOfPatients[j])}</td>
      ]
    })
    return <tr key={i}>
      <td>{a.category}</td>
